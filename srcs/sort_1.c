...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/01/20 04:36:26 by jyeo              #+#    #+#             */
/*   Updated: 2018/01/20 04:36:30 by jyeo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/push_swap.h"

static int	ft_sort1_part1(t_lst *lst, int zone, int sub_zone, char *location)
{
...
static int	ft_sort1_part2(t_lst *lst, int zone, int sub_zone, char *location)
{
...
int			ft_sort1(t_lst *lst, int zone, int sub_zone)
{
	...
	{
		ft_check_current(lst->a, lst->current, zone, &location);
		if (lst->current == (rank.max + 1) && (lst->b->zone == (zone + 1)))
			round = 1;
		else
		{
			if (lst->a && location == 'a')
				ft_sort1_part1(lst, zone, sub_zone, &location);
			else if (lst->b && location == 'b')
				ft_sort1_part2(lst, zone, sub_zone, &location);
		}
	}
	return (0);
}
